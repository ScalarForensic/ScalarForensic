...
**Configuration stability after indexing:** `SFN_SSCD_N_CROPS` and `SFN_NORMALIZE_SIZE` are baked into every embedding and must not change after a collection has been indexed. `sfn-web` will refuse to start if these settings differ from what was used during indexing; `sfn` will refuse to index unless `--ignore-config-mismatch` is passed. The model paths (`SFN_MODEL_DINO`, `SFN_MODEL_SSCD`) are validated by model hash during `sfn index` but are not re-checked at web-server startup — changing models without re-indexing will silently corrupt similarity results. To change any of these, re-create the Qdrant collection and re-index from scratch.
