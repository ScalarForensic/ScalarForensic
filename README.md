...
**Configuration stability after indexing:** `SFN_SSCD_N_CROPS`, `SFN_NORMALIZE_SIZE`, and the model paths (`SFN_MODEL_DINO`, `SFN_MODEL_SSCD`) are baked into every embedding and must not change after a collection has been indexed. The `sfn` indexer enforces all three: a mismatch against existing collection points aborts the run (override with `--ignore-config-mismatch`). The `sfn-web` server enforces only `SFN_SSCD_N_CROPS` — the single Phase-2 setting whose drift silently corrupts query results, since queries are re-embedded with the *current* number of crops and compared against vectors stored with the *original* number. `SFN_NORMALIZE_SIZE` and the model files are caught at the next `sfn index` run rather than at server startup. To change any of these settings, re-create the Qdrant collection and re-index from scratch.
